...
Reads credentials from strava_tokens.json (created by strava_auth.py).
On first run fetches all activities. On subsequent runs only fetches
new activities since the last fetch.
...
Output:
    strava_activities.csv     — one row per activity
    strava_splits_100m.csv    — one row per 100m segment
"""
...
def load_tokens():
    if not Path(TOKEN_FILE).exists():
        raise FileNotFoundError(f"'{TOKEN_FILE}' not found. Run strava_auth.py first.")
...
def api_get(endpoint, access_token, params=None, retries=3):
    ...
    for attempt in range(retries):
        ...
        if resp.status_code == 404:
            return None
        resp.raise_for_status()
        time.sleep(0.25)  # stay well within 100 req/15min
        return resp.json()
    return None
...
# ---------------------------------------------------------------------------
# Build 100m splits from streams
# ---------------------------------------------------------------------------

def interpolate(series, target_dist, dist_series):
    """Linear interpolation of a value series at a target distance."""
...
def build_100m_splits(activity_id, start_date_local, streams):
    """
    Convert raw GPS streams into 100m split rows.
    Returns list of dicts, one per 100m segment.
    """
    ...
    while target <= total_dist + SPLIT_INTERVAL:
        ...
        # Interpolate values at start and end of segment
        def interp(series):
            if series is None:
                return None
            start_val = interpolate(series, seg_start, dist_data)
            end_val   = interpolate(series, seg_end,   dist_data)
            return start_val, end_val

        t_vals  = interp(time_data)
        hr_vals = interp(hr_data)
        alt_at_start = interpolate(alt_data, seg_start, dist_data) if alt_data else None
        alt_at_end   = interpolate(alt_data, seg_end,   dist_data) if alt_data else None

        # Segment duration
        seg_time_s = None
        if t_vals and t_vals[0] is not None and t_vals[1] is not None:
            seg_time_s = t_vals[1] - t_vals[0]

        seg_dist = seg_end - seg_start

        # Average HR across the segment (average of sampled points within window)
        avg_hr = None
        if hr_data and time_data:
            in_seg = [
                hr_data[i] for i in range(len(dist_data))
        ...
        if cad_data:
            in_seg = [
                cad_data[i] for i in range(len(dist_data))
        ...
        if vel_data:
            in_seg = [
                vel_data[i] for i in range(len(dist_data))
        ...
        # Elevation change
        elev_change = None
        if alt_at_start is not None and alt_at_end is not None:
            elev_change = round(alt_at_end - alt_at_start, 2)
        ...
        splits.append({
            ...
            "altitude_m":        round(alt_at_start, 1) if alt_at_start is not None else None,
            "start_lat":         start_lat,
            "start_lng":         start_lng,
        })
...
def load_existing_split_ids(filepath):
    if not Path(filepath).exists():
        return set()
    with open(filepath, newline="", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        return {row["activity_id"] for row in reader if row.get("activity_id")}
...
def main():
    parser = argparse.ArgumentParser(description="Fetch Strava activities + 100m splits.")
    ...
    print(f"\nFetching for: {tokens.get('athlete_name', 'unknown')}\n")
    ...
    if args.all:
        ...
        print(f"Mode: last {args.days} days")
    else:
        state = load_state()
        last_fetch = state.get("last_fetch_epoch")
        if last_fetch:
            dt = datetime.fromtimestamp(last_fetch)
            print(f"Mode: incremental since {dt.strftime('%Y-%m-%d %H:%M')}")
            after_epoch = last_fetch
        else:
            print("Mode: first run — fetching everything")
    ...
    # --- Step 2: write activity summaries ---
    print("\n[2/3] Writing activity summaries...")
    existing_act_ids = load_existing_ids(ACTIVITIES_FILE)
    ...
    # --- Step 3: fetch streams + build 100m splits ---
    print(f"\n[3/3] Fetching GPS streams + building 100m splits...")
    existing_split_ids = load_existing_split_ids(SPLITS_FILE)
    ...
    for i, a in enumerate(activities):
        ...
        # Skip non-GPS activities (indoor, manual)
        ...
        if splits:
            ...
            print(f"    → {len(splits)} splits ({splits[-1]['split_end_m']:.0f}m total)")
        else:
            print(f"    → No splits generated.")

    # Write all splits in one go
    n_splits = append_rows(all_splits, SPLITS_FILE)
    print(f"\n  ✓ {n_splits} split rows written to {SPLITS_FILE}")
    if skipped:
        print(f"  ({skipped} activities already had splits, skipped)")
    if no_stream:
        print(f"  ({no_stream} activities had no GPS stream)")

    save_state({"last_fetch_epoch": int(time.time())})
    print(f"\nDone. Files ready for analysis:")
    print(f"  {ACTIVITIES_FILE}")
    print(f"  {SPLITS_FILE}")
